...
word_frequencies_filtered = {word: tokens.count(word) for word in tokens if tokens.count(word) > 1}
...
# Your code here:
# -----------------------------------------------
def token_counts(input_string: str, k: int = 1) -> dict:
    ...
    filtered_tokens = {token: count for token, count in token_freq.items() if count > k}
...
# Your code here:
tokens = ['dog', 'quick', 'fox', 'the', 'over', 'lazy', 'brown', 'jumps']
...
def tokenize_and_encode(documents: list) -> tuple:
    # Step 1: Create the token_to_id and id_to_token dictionaries
    token_to_id, id_to_token = make_vocabulary_map(documents)
    
    # Step 2: Encode each document using the token_to_id dictionary
    encoded_sentences = []
    for doc in documents:
        tokens = tokenize(doc)  # Tokenize the document
        encoded = [token_to_id[token] for token in tokens]  # Convert tokens to IDs
        encoded_sentences.append(encoded)  # Add encoded sentence to the list
    
    # Step 3: Return the encoded sentences and the mappings
