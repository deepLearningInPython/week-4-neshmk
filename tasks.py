...
tokens = [word.strip(punctuation) for word in text.split()]
...
def tokenize(string: str) -> list:
    ...
    # Use list comprehension to tokenize, remove punctuation, and convert to lowercase
...
# -----------------------------------------------
# Tokenize text
tokens = [word.strip(punctuation).lower() for word in text.split()]

word_frequencies = {word: tokens.count(word) for word in set(tokens)} #set() gives unique words in the list of tokens
...
def token_counts(string: str, k: int = 1) -> dict:
    ...
    token_freq = {word: count for word, count in word_frequencies.items() if count >= k}
...
# Your code here:
# -----------------------------------------------
token_to_id = {token: idx for idx, token in enumerate(set(tokenize(text)))}
